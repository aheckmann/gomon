...
module.exports = exports = function (db, term) {
    ...
    db.executeDbAdminCommand({ listDatabases: 1 }, {}, function (err, dbs) {
      ...
      if (rich) {
        ...
        names.forEach(function (n) {
          // autocomplete
          //term.context.use['db_' + n] = function () {
            //return term.context.use(n);
          //}

          var padding = new Array(width - n.length).join(' ');
          if (size[n] > 1) {
            console.log(n + padding + size[n] / 1024 / 1024 / 1024 + "GB");
          } else {
            console.log(n + padding + "(empty)");
          }
        });
      }
      term.displayPrompt();
